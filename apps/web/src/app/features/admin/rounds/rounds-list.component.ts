    ...
    <div class="rounds-list">
      ...
      <div class="page-header">
        ...
        <div class="header-actions">
          ...
          <button class="btn btn-primary" (click)="createRound()" [disabled]="isLoading">
            ✨ Nouveau Round
          </button>
        </div>
      </div>
        ...
        >
          <div class="round-header">
            ...
            <div class="round-actions" (click)="$event.stopPropagation()">
              <button class="action-btn" (click)="editRound(round)" title="Modifier">✏️</button>
              <button class="action-btn delete" (click)="deleteRound(round)" title="Supprimer">
                🗑️
              </button>
            </div>
          </div>
          ...
          <div class="round-progress">
            <div class="progress-bar">
              <div class="progress-fill" [style.width.%]="getProgressPercentage(round)"></div>
            </div>
          ...
          <div class="round-footer">
            <span class="round-date"> Créé le {{ formatDate(round.created_at) }} </span>
...
export class RoundsListComponent implements OnInit {
  ...
  loadRoundsWithCode(eventCode: string) {
    ...
    this.http.get<{ rounds: Round[] }>(`/api/events/${eventCode}/rounds`, { headers }).subscribe({
      next: (response) => {
  ...
  loadSongCounts() {
    this.rounds.forEach((round) => {
      this.http.get<{ count: number }>(`/api/rounds/${round.id}/songs/count`).subscribe({
        next: (response) => {
  ...
  deleteRound(round: Round) {
    if (confirm(`Êtes-vous sûr de vouloir supprimer le round "${round.name || round.id}" ?`)) {
      this.http.delete(`/api/rounds/${round.id}`).subscribe({
        next: () => {
